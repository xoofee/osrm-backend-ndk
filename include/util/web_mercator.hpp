...
const constexpr double MAXEXTENT = EARTH_RADIUS_WGS84 * M_PI;
...
inline void pixelToDegree(const double shift, double &x, double &y)
{
    ...
    const double g = (y - b) / -(shift * 0.5 * (1/M_PI)) / detail::DEGREE_TO_RAD;
    static_assert(detail::DEGREE_TO_RAD * 0.5 * (1/M_PI) - 1 / 360. < 0.0001, "");
