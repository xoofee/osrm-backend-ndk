...
extern "C" int cmain(int argc, const char *argv[]) {    // to be used by dart::ffi
    try
    {
...
extern "C" JNIEXPORT jint JNICALL Java_com_example_myapplication4_MainActivity_cmain(JNIEnv *env, jobject thiz, jint argc, jobjectArray argv) {
    (void)thiz; 
    // Convert the Java String array to a C-style string array (char **argv)
    const char **c_argv = new const char*[argc];
    ...
    int result = cmain(argc, c_argv);  // Call your function
