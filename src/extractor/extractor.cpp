                                                 ...
                                                 const unsigned number_of_threads)
{
    ...
    // auto registered_xml_parser = osmium::io::detail::get_registered_xml_parser();
    // (void) registered_xml_parser;

    // std::cout << "registering xml parser in extractor.cpp: " << std::endl;
    // osmium::io::detail::ParserFactory::instance().register_parser(
    //     osmium::io::file_format::xml,
    //     [](osmium::io::detail::parser_arguments& args) {
    //         return std::unique_ptr<osmium::io::detail::Parser>(new osmium::io::detail::XMLParser{args});
    // });

    // osmium::io::CompressionFactory::instance().register_compression(osmium::io::file_compression::none,
    //     [](const int fd, const osmium::io::fsync sync) { return new osmium::io::NoCompressor{fd, sync}; },
    ...
    // util::FloatCoordinate source_coord(FloatLongitude{103.928912}, FloatLatitude{1.307196});
    // util::FloatCoordinate target_coord(FloatLongitude{103.928700}, FloatLatitude{1.307091});

    //         util::Log()  << "a3 " << std::flush;    

    // void* p = util::coordinate_calculation::getCheapRulerContainer();

    // std::cout << "pointer: " << (void*)p  << std::endl;

    // const auto distance =
    //     util::coordinate_calculation::greatCircleDistance(source_coord, target_coord);
    //         util::Log()  << "a4 " << distance << std::flush;    
    ...
    { // Parse OSM header
        ...
        util::Log() << "after reader creation";
